...
// signatureSurfaceFlags is the flag union a command that verifies signatures
// mounts: the collection flags, the signature flags, and the S3 cache flags.
// No command mounts it today, which is exactly why these tests exist - they
// are what keeps cliflags.SignatureFlags exercised until one does.
func signatureSurfaceFlags() []cli.Flag {
	flags := cliflags.CollectionFlags()
	flags = append(flags, cliflags.SignatureFlags()...)

	return append(flags, cliflags.S3Flags()...)
}
...
// runInstallWith runs an install command carrying flags and returns whatever
...
func runInstallWith(t *testing.T, flags []cli.Flag, args []string) error {
	t.Helper()
	...
	return app.Run(context.Background(), append([]string{"go-galaxy", "install"}, args...))
}
...
// argument: adding SignatureFlags makes the identical command line parse, so
// the refusal above is the flag set's doing rather than the harness rejecting
// everything.
func TestSignatureFlagsAreNotPartOfCollectionFlags(t *testing.T) {
	neutralizeAnsibleDiscovery(t)

	t.Run("collection flags alone refuse it", func(t *testing.T) {
		err := runInstallWith(t, cliflags.CollectionFlags(), []string{"--keyring=/keys.gpg"})
		if err == nil {
	...
	t.Run("adding the signature flags accepts it", func(t *testing.T) {
		if err := runInstallWith(t, signatureSurfaceFlags(), []string{"--keyring=/keys.gpg"}); err != nil {
			t.Fatalf("app.Run() error = %v, want nil", err)
		}
	})
}

// TestCleanupSignatureDefaults pins the resolution for a command that
// registers none of these flags - cleanup is the one that exists today, and
// every command is that shape until one mounts them. The count must still be
// a spec the grammar accepts, since BuildCollectionConfig validates it for
// every command regardless of which flags that command declared.
