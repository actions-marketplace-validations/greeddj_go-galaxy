...
go-galaxy is a fast Ansible Galaxy collections and roles installer for CI, written in Go (module `github.com/greeddj/go-galaxy`). Collections come from Galaxy API servers and from git repositories; roles come from the Galaxy v1 role API (then fetched by git at the tag it names) and from git repositories, and install under `roles_path` (`--roles-path`, default `.roles`). One `install` handles both lists of one requirements.yml; `url`, `file`, `dir` and tarball sources are refused. `vendor/` is git-ignored, not committed: it is regenerated locally by `just deps` and is absent in CI, so a local build resolves through it while CI downloads modules.
...
- `internal/galaxy/lockfile`: pins every transitive collection to exact version + SHA256, or, for a git source, to its commit (`type: git`, `ref`, `commit`, `subdir`, no sha256), and every role (a `roles:` list of `RoleEntry`: name, `type: galaxy|git`, version as asked, galaxy name, source, repository, ref, commit, deps, no sha256); a file holding a role is schema 3, one holding a git entry and no role is schema 2, one with neither stays schema 1 byte for byte. With a lockfile, install reads only the cache, never the Galaxy API or the remote; a cache miss on a role fetches its pinned commit and refuses a remote that serves another (`ErrRoleArtifactIdentityMismatch`).
