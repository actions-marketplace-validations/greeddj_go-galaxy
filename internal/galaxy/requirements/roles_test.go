...
func roleRejectedCases() []roleRejectedCase {
	return []roleRejectedCase{
		...
		{name: "tarball url", input: "roles:\n  - src: https://example.com/r.tar.gz\n", wantErr: helpers.ErrUnsupportedRoleSource},
		{name: "github tarball url", input: "roles:\n  - src: https://github.com/a/r/archive/1.tar.gz\n",
			wantErr: helpers.ErrUnsupportedRoleSource},
...
func TestParseRoleDependency(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "tarball", dep: gitsource.RoleDependency{Src: "https://example.com/r.tar.gz"}, wantErr: helpers.ErrUnsupportedRoleSource},
