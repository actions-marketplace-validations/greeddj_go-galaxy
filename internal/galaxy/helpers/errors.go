...
var (
	...
	// lock would still overwrite a version-controlled lockfile - and it would
	// still print its normal success output, with nothing marking the run as
	// a dry run at all. Refusing outright beats proceeding: a loud, immediate
	// usage error is recoverable by dropping the flag, while a silent full
	// mutation announced as a dry run is not. install, warm, and cleanup do
	// implement --dry-run; outdated needs no guard at all, since a read-only
	// command has no product for --dry-run to suppress in the first place.
