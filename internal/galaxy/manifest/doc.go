...
// downloaded or produced and only ever reads it, handing back bytes it found or
// a verdict on what it read, so nothing in this package can create, replace or
// delete a byte anywhere - not under a project's collections tree, not under
// the cache, not even a temporary file of its own. The documents read here
// decide whether an install proceeds, so the code reading them is deliberately
// unable to change what is installed, the same separation
