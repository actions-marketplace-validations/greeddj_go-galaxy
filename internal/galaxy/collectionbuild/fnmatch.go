...
// ignoreRules is the build's exclusion list: ansible's defaults, then
// build_ignore in the order written, each matched with Fnmatch against the
// "/"-joined path relative to the collection root - so "galaxy.yml" and
// "tests/output" match at the root only - plus the basename prune list that
// applies to directories alone.
type ignoreRules struct {
	dirNames map[string]struct{}
	patterns []string
}

func newIgnoreRules(namespace, name string, buildIgnore []string) ignoreRules {
	...
	return ignoreRules{patterns: patterns, dirNames: ignoreDirNames()}
}

// skip reports whether the entry at rel, a clean "/"-joined path relative to
// the collection root, is left out of the build.
func (r ignoreRules) skip(rel string, isDir bool) bool {
	if isDir {
...
func (r ignoreRules) excludes(rel string, isDir bool) bool {
