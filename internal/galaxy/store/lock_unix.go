...
// for a live lock. The returned closure releases the lock and closes the
// file descriptor exactly once.
func flockFile(lockPath string) (func() error, error) {
	...
	f, err := os.OpenFile(lockPath, os.O_RDWR|os.O_CREATE, helpers.FileMod)
	if err != nil {
		return nil, err
	}
