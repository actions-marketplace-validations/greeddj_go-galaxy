...
- **`govulncheck`** in `just check` runs at full strictness (no exit-code masking). Findings will fail the gate. Its verdict depends on the date as well as on the commit - a newly published advisory against a path this module reaches turns an unchanged tree red - so read the finding and then bump the dependency, bump `go.mod`'s `go` directive when the finding is against the standard library, or make the path unreachable; never filter the output or mask the exit code.
- **Dependency bumps arrive on their own, except the versions the gates pin.** [.github/dependabot.yml](.github/dependabot.yml) opens weekly `gomod` and `github-actions` PRs (each ecosystem's minor and patch updates grouped into one, majors separate). A `gomod` PR carries `go.mod`/`go.sum` only, since `vendor/` is git-ignored, so run `just deps` after merging one or the next `just check` fails on Go's vendor-consistency error; a `ci(deps)` PR edits `.github/workflows/*.yml` and needs none of that. Two versions the gates depend on are invisible to the bot, since nothing resolves either as a dependency: the pinned golangci-lint release (a `Justfile` literal plus a workflow step input, neither a module requirement nor an action reference) and `go.mod`'s `go` directive (what `setup-go` pins CI's toolchain to, and what a standard-library `govulncheck` finding is bumped through). Both are bumped by hand - the linter in both spellings in one commit, as above.
