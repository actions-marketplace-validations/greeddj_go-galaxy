...
- `outdated` (`o`) - compare each lockfile entry against the latest version on its Galaxy server, or, for a git entry, the locked commit against what its ref points at now; a url entry is reported as current by construction, since its content-addressed pin has no version feed to compare against (drift is `--refresh` and `--frozen` territory). A locked role is reported on a line whose name carries a `role` prefix: a git role is compared by commit like a git collection; a Galaxy role is compared against the highest tag the v1 API lists today, by name rather than by semver order (role tags are not held to semver), and a role the server lists no tags for is compared by its default branch's commit. See [outdated](#outdated) below for what it does not do: no cache backend, no lock, no cached answer.
